...
using Depra.Data.Operations.Cache.Bridge;
...
namespace Depra.Data.Storage.Tests
{
    ...
    internal class CacheDataStorageTests
    {
        ...
        {
            var cacheLocation = new CacheLocation();
            var storage = StandardDataStorageBuilder
                .Configure(cacheLocation, builder => builder
                    .AddSaver(saver => saver.AddWriter(new DataWriter<TestData>(cacheLocation)))
                    .AddLoader(loader => loader.AddReader(new DataReader<TestData>(cacheLocation))))
                .Build();
        ...
        private void WarmUpData(IEnumerable<string> dataNames)
        {
            ...
            {
                _cacheDataStorage.SaveData(dataName, new TestData());
            }
        }
    }
}